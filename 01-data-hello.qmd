...
Only about 14% of 10,000 simulated seasons produce exactly 10 goals, and the tallies range from 1 all the way to 25 -- from "release her" to "sell for a fortune," with no change whatsoever in the underlying player.
...
However, this study does leave us with an important lesson, one every scouting department should frame on the wall: we should keep our eyes open for surprises.
...
The cloud drifts downward from left to right: teams whose shot profile contains more headers tend, loosely, to convert at lower rates.
...
Independence is a property of the underlying population or process, not something you can read off a single plot: two variables are independent if knowing the value of one tells you nothing about the distribution of the other.
